...
with open(r'.\testjson_1\gt.json') as f:
    gt = json.loads(f.read())

with open(r'.\testjson_1\result.json') as f:
    result = json.loads(f.read())

type45 = ['danger', 'prohibitory', 'mandatory', 'other']
...
imgid = list(range(600, 900))

summary = eval_annos(gt, result, iou=0.5, imgids=None, check_type=True, types=type45, minscore=40, minboxsize=0, maxboxsize=32, match_same=True)
print('small recall:' + str(summary["recall"]))
print('small accuracy:' + str(summary["accuracy"]))

summary = eval_annos(gt, result, iou=0.5, imgids=None, check_type=True, types=type45, minscore=40, minboxsize=32, maxboxsize=96, match_same=True)
print('medium recall:' + str(summary["recall"]))
print('medium accuracy:' + str(summary["accuracy"]))

summary = eval_annos(gt, result, iou=0.5, imgids=None, check_type=True, types=type45, minscore=40, minboxsize=96, maxboxsize=400, match_same=True)
